...
class Memory:
    ...
    async def process(
        self, request: ProcessRequest, *, extraction_timeout: float = 30.0
    ) -> str:
        ...
        Args:
            request: The transcript and entities to process.
            extraction_timeout: Maximum seconds the background task may
                run before being marked ``"failed"`` with a timeout.
        """
        ...
        _ = asyncio.create_task(
            self._process_background(event_id, request, extraction_timeout)
        )
        return event_id

    async def _process_background(
        self, event_id: str, request: ProcessRequest, extraction_timeout: float
    ) -> None:
        """Async pipeline: extract → dedup → store → synthesize.

        Runs as a fire-and-forget :class:`asyncio.Task`. The entire
        pipeline is wrapped in :func:`asyncio.wait_for` so a hung LLM
        call does not block the task forever.
        """
        try:
            await asyncio.wait_for(
                self._run_extraction(event_id, request),
                timeout=extraction_timeout,
            )
        except TimeoutError:
            await self._fail_event(
                event_id, f"extraction timed out after {extraction_timeout}s"
            )
        except Exception as exc:
            logger.exception("Background extraction %s failed", event_id)
            await self._fail_event(event_id, str(exc))

    async def _run_extraction(
        self, event_id: str, request: ProcessRequest
    ) -> None:
        """Core extraction pipeline. See :meth:`_process_background`."""
    ...
    async def _fail_event(self, event_id: str, error: str) -> None:
